...
public class Transporter: IDisposable {
    ...
    public Transporter(NetClient nc,Socket netSocket) {
        ...
        if (nc == null || netSocket == null) {
            Console.WriteLine("netclient or socket is null");
            return;
        }
        this.nClient = nc;
    ...
    void RevTCP() {
        while (true) {
            try {
                ...
                Console.WriteLine("revTcp len" + len + "revCache read:" + revCache.ReadPtr + "revCache WritePtr:" + revCache.WritePtr);
                if (len > 0) {
                ...
                Console.WriteLine("rev exception :" + exp);
                break;
            }
        }
    }
    ...
    void RevUDP() {
        while (true) {
            try {
                ...
                Console.WriteLine("revUdp len" + len);
                if (len > 0) {
                    revCache.Write(buffer, len);
                    ReceiveData rd;
                    do {
...
#if net_log
        Console.WriteLine("decode read first 2 byte packageLength:" + packageLength);
#endif
        if (packageLength > Transporter.MAX_PACKAGE_LENGTH) {
            Console.WriteLine("Transporter receive package length over max length :" + packageLength);
            cache.ReadPtr = startPtr;
            return null;
        }
        if(cache.Length < packageLength) {
            Console.WriteLine("decode packageLenght over cache length packageLength:" + packageLength + "cache.Length:" + cache.Length);
            cache.ReadPtr = startPtr;
            return null;
        }
        UInt16 typeId = cache.ReadUInt16();
#if net_log
        Console.WriteLine("decode typeId:" + typeId);
#endif
        Type packageType = ClientProtocol.GetTypeById(typeId);
        if (packageType == null) {
            Console.WriteLine("Transporter receive package can not find id" + typeId);
            return null;
        }
    ...
    public bool Send(object msg) {
        try {
            ...
            Console.WriteLine("transporter send exception :" + e.ToString());
            return false;
        }
    }
...
#if net_log
        Console.WriteLine("encode id is :" + msgId);
#endif
        id = msgId;
...
#if net_log
        Console.WriteLine("encode after Serialize:" + memoryStream.Position);
#endif
